...
@Parameters(
        commandNames = "inventory-download",
        commandDescription = "Initiate an inventory download, and it will download the last successful InventoryRetrieval")
public class CommandInventoryDownload extends GenericCommand {
    ...
    @Parameter(required = true, names = "--journal",
            description = "Where to save the data retrieved from the inventory")
    ...
    @Parameter(
            names = "--raw",
            description = "Store the glacier full data instead of the parsed one, useful for creating new metadata parsers")
