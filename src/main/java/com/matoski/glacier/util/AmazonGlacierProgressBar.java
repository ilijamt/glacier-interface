...
public class AmazonGlacierProgressBar {
    ...
    public void process(int done, int total) {
        ...
        char[] workchars = {'|', '/', '-', '\\'};
        /**
         * it is the progress bar format. let's have a closer look: - \r is the carriage return, in
         * other words it moves the "cursor" to the first position on the same line - %3d is a 3 digit
         * decimal integer - %% is the literal % character - %s is a string, particularly the "###...#"
        ...
        /**
         * if the work have been completed it forces the output to be written, changes line and resets
         * itself so it can be reused.
         */
        if (done == total) {
            System.out.flush();
            System.out.println();
