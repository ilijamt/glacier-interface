...
public class ListJournalCommand extends AbstractEmptyCommand<CommandListJournal> {
	...
	for (Entry<String, Archive> entry : journal.getArchives().entrySet()) {
		...
		System.out.println(String.format("%1$20s: %2$s (%3$s bytes)", "Size", FileUtils.humanReadableByteCount(archive.getSize()), archive.getSize()));
		System.out.println(String.format("%1$20s: %2$s (%3$s)", "Modified Date", new Date(archive.getModifiedDate()), archive.getModifiedDate()));
		if (command.exists) {
		    System.out.println(String.format("%1$20s: %2$s", "Exists", file.exists() && file.isFile()));
		}
