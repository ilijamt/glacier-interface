...
public class ListVaultJobsCommand extends AbstractCommand<CommandListVaultJobs> {
	...
	try {
		...
		for (GlacierJobDescription job : jobs) {
			...
			System.out.println(String.format("%s [Action: %s] %s (Completed: %s, On: %s) ID: %s", job.getCreationDate(),
				job.getAction(), job.getStatusCode(), job.getCompleted(), job.getCompletionDate(), job.getJobId()));
		    }
		}
	    }
		...
		break;
	    default:
		System.err.println(String.format("ERROR: Failed to retrieve the jobs for vault: %s\n\t%s", command.vaultName,
			e.getMessage()));
		break;
	    }
